            ...
            {products.map((product) => (
                <div key={product.id} className={`flex shrink-0 m-6 relative  overflow-hidden ${product.bgColor} rounded-lg shadow-lg group max-w-sm`}>
                    ...
                    <div className="relative pt-10 px-10 flex items-center justify-center group-hover:scale-110 transition-transform">
                        ...
                        <div style={{ background: "radial-gradient(black,transparent 60%", transform: 'rotate3d(0,0,1,20deg) scale3d(1, 0.6,1)', opacity: 0.2, }} className="block absolute w-48 h-48 bottom-0 left-0 -mb-24 ml-3">

                        </div>
                        <Image className="relative w-52" src={product.imageUrl} alt={product.title} />
                    </div>
